...
enum Col:
  override def toString: String =
    this match {
      case Col.StringCol(value)   => value
      case Col.NumberCol(value)   => value.toString
      case Col.DateTimeCol(value) => value.toString
    }
  case StringCol(value: String)
  case NumberCol(value: Number)
  case DateTimeCol(value: LocalDateTime)
  ...
  def apply(model: TableModel): Html[Msg] = {
    ...
    div(cls := "relative overflow-x-auto shadow-md sm:rounded-lg")(
      table(cls := "relative overflow-x-auto shadow-md sm:rounded-lg")(
        tr(
          model.colHeaders.map(colHeader => th(cls := "text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400")(colHeader))
        ),
        tbody(
          model.rows.map(row =>
            currentRow += 1

...
lazy val frontend = (project in file("frontend"))
  ...
  .settings(
    ...
    dockerAlias           := DockerAlias.apply(dockerRepository.value, None, "tryflowbite", dockerAlias.value.tag.map(t => "frontend-" + t)),
...
lazy val backend = (project in file("backend"))
  ...
  .settings(
    ...
    dockerRepository   := Some(DockerSettings.repository),
    dockerAlias        := DockerAlias.apply(dockerRepository.value, None, "tryflowbite", dockerAlias.value.tag.map(t => "backend-" + t)),
    DockerSettings.x86ArchSetting
  )
...
lazy val common = (crossProject(JVMPlatform, JSPlatform)
  ...
  .settings(
    ...
    buildInfoPackage := "com.example.tryflowbite.common"
  )
